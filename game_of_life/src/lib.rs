...
//! So what you have to do is just create an automata, set some cells (to make them alive) and to see how it evoluates !\
//! ***Have fun !***
mod ansi_rust;
pub use ansi_rust::*;
        ...
        for ilig in 1..nb_lig - 1 {
            for icol in 1..nb_col - 1 {
                ...
                for i in 0..3 {
                    for j in 0..3 {
                        if !(i == 1 && j == 1)
                            && self.grid[(ilig + j - 1) as usize][(icol + i - 1) as usize]
                        {
                            nb_vies += 1;
                        }
                    }
